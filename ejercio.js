...
const contenedorPersonal = document.querySelector("#contenedorPersonal");
const botonesSelect = document.querySelectorall(".botones-select");

const cargarPersonal = () => {
    contenedorPersonal.innerHTML = ''; // Limpiar el contenedor antes de llenarlo
    empleados.forEach(empleado => {
        let div = document.createElement("div");
        div.classList.add("empleado");
        div.innerHTML = `
            <img src="${empleado.imagen}" alt="Imagen de ${empleado.nombre}">
            <h1>Nombre: ${empleado.nombre}</h1>
            <h2>Sector: ${empleado.categoria}</h2>
            <p>Turno: ${empleado.turno}</p>
            <button class="personalAgregar" id="${empleado.id}">Agregar</button>
        `;
        contenedorPersonal.appendChild(div);
    });
};
cargarPersonal()
...
botonesSelect.forEach(boton => {
    boton.addEventListener("click", (e) => {
        ...
        botonesSelect.forEach(btn => btn.classList.remove("active"));

        // Añadir la clase active al botón clicado
        e.currentTarget.classList.add("active");

        // Cargar el personal solo si el botón clicado es el de empleados
